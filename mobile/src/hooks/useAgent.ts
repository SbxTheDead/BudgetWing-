...
import { agentEndpoint } from "../lib/api";
import { getCity } from "../lib/cities";
import { parseTripRequest } from "../lib/parse";
 ...
 * fetch + ReadableStream (not EventSource) because the planner takes a POST.
 */
...
export function useAgent() {
  ...
  const push = useCallback((item: Omit<ChatMessage, "id" | "timestamp">) => {
    setMessages((prev) => [
      ...prev,
        ...
        for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
          try {
            ...
            push({
              role: "agent",
              kind: "error",
              content: `Lost the connection to the planner (${reason}). Check the API URL in Settings and retry.`,
            });
          }
        }
      } finally {
  ...
  const retry = useCallback(async () => {
    ...
    setMessages((prev) => prev.filter((item) => item.kind !== "error"));
