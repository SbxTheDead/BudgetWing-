/**
 * Airport registry used by the map and by free-text request parsing.
 * Coordinates are the airport itself, so arcs land on the runway, not downtown.
 *
 * Every code the agent can resolve (`agent/tools/airports.ts`) has to exist here
 * too, otherwise a leg the agent picked — including a nearby-airport swap like
 * BKK→DMK — would silently vanish from the map.
 */
...
/** Great-circle distance in km — drives the mock pricing model. */
export function distanceKm(a: string, b: string): number {
  const ca = getCity(a);
  const cb = getCity(b);
  if (!ca || !cb) return 1200;
  const R = 6371;
