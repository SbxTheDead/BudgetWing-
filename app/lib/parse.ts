...
/**
 * Free-text → TripRequest. Built for the demo phrasing
 ...
 * so the UI can ask a follow-up question instead of guessing silently.
 */
...
function parseFlex(text: string): number {
  ...
  if (/flexible|whenever|any dates?/i.test(text)) return 3;
  return 3;
}
...
/** Human-readable summary echoed back into the chat transcript. */
export function describeRequest(req: TripRequest): string {
  const names = req.cities.map((c) => getCity(c)?.city ?? c);
  return `${names.join(" → ")} · ${req.startDate} to ${req.endDate} · ±${req.flexDays}d · ${req.passengers} pax`;
}
